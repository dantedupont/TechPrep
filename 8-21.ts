...
type AssignmentRecord = Record<string, GradeData>
type GradeData = {
    maxPoints: number,
    grades: Record<string, number>
}

function find_passing_students(assignments: AssignmentRecord): string[] {
    let totalPoints: number = 0
    const studentGrades = new Map<string,number>()
    for(const assignmentName in assignments){
        totalPoints += assignments[assignmentName]!.maxPoints
        const gradeData = assignments[assignmentName]!.grades
        for(const studentName in gradeData){
            const currentTotal = studentGrades.get(studentName) ?? 0
            studentGrades.set(studentName, currentTotal + gradeData[studentName]!)
        }
    }
    const passingStudents: string[] = [...studentGrades]
        .filter(([studentName, studentTotal]) => studentTotal / totalPoints > .7)
        .map(([studentNames]) => studentNames)
...
console.log(find_passing_students(assignments))